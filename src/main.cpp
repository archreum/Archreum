...
#if defined(NDEBUG)
# error "Archreum cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xe18722c99dfbb65537a8a182240843bfbbe05d9208d6ac4592c7434b504ea7a1");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Archreum: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Archreum Signed Message:\n";
...
{
    // Archreum: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Archreum: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Archreum: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1524848429, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "Itisbestthingtomeetmydaughter";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 1000 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("045d0df8a13ced77823c62925fc7861bb4c29d76e4e4a4afcc3923c3472a2f466c93798e394f56ec45b7cb80f37a96d1814f1f8fac1b85415c98ffb4c8b914c030") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1524848429;
        ...
        assert(block.hashMerkleRoot == uint256("0xaa669cb38f2493330370f163416b5aebc7db02ea05e8daddd2b8fe66b4e11640"));
...
unsigned char pchMessageStart[4] = { 0x6c, 0x60, 0xb0, 0xb3 }; // Archreum: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ArchreumMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ArchreumMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ArchreumMiner(CWallet *pwallet)
{
    printf("ArchreumMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("archreum-miner");
    ...
    try { loop {
        ...
        printf("Running ArchreumMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ArchreumMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ArchreumMiner, pwallet));
}
